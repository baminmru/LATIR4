...
Module MakeRowProc
    ...
    Public Sub MakeRow(ByRef tid As String, ByRef m As MTZMetaModel.MTZMetaModel.Application, ByRef ot As MTZMetaModel.MTZMetaModel.OBJECTTYPE, ByRef p As MTZMetaModel.MTZMetaModel.PART, ByRef o As LATIRGenerator.Response)
        ...
        s = s & vbCrLf & "        Inherits LATIR.Document.DocRow_Base"
        s = s & vbCrLf & ""
        ...
        For i = 1 To p.FIELD.Count
            ...
            If ft.TypeStyle <> MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Element_oformleniy Then
                ...
                If LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, f.FieldType.ID, tid, True) <> String.Empty Then
                    ...
                    s = s & vbCrLf & "            private m_" & f.Name & "  as " & LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, f.FieldType.ID, tid, True)
                End If
            End If
        Next
            ...
            If ft.TypeStyle <> MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Element_oformleniy Then
                ...
                ElseIf ft.TypeStyle = MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Perecislenie Then
                    ...
                    For j = 1 To ft.ENUMITEM.Count
                        s = s & vbCrLf & "            case " & LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True) & "." & LATIRFramework.FieldTypesHelper.MakeValidName(ft.Name) & "_" & LATIRFramework.FieldTypesHelper.MakeValidName(ft.ENUMITEM.Item(j).Name)
        ...
        s = s & vbCrLf & "        Public Overrides Function FindInside(ByVal Table As String, ByVal RowID As String) As LATIR.Document.DocRow_Base"
        s = s & vbCrLf & "            dim mFindInside As LATIR.Document.DocRow_Base = Nothing"
        ...
        s = s & vbCrLf & "        Public Overrides Sub Pack(ByVal nv As LATIR.NamedValues)"
        ...
        For i = 1 To p.FIELD.Count
            ...
            If ft.TypeStyle <> MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Element_oformleniy Then
                If ft.TypeStyle = MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Ssilka Then
                    ...
                    s = s & vbCrLf & "          end if "
                Else
                    If UCase(LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True)) = "DATE" Then
                        s = s & vbCrLf & "          if " & f.Name & "=System.DateTime.MinValue then"
                        s = s & vbCrLf & "            nv.Add(""" & f.Name & """, system.dbnull.value, " & LATIRFramework.FieldTypesHelper.MapDBtype(LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True)) & ")"
                        s = s & vbCrLf & "          else"
                        s = s & vbCrLf & "            nv.Add(""" & f.Name & """, " & f.Name & ", " & LATIRFramework.FieldTypesHelper.MapDBtype(LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True)) & ")"
                        s = s & vbCrLf & "          end if "
                    Else
                        If LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True) <> String.Empty Then
                            s = s & vbCrLf & "          nv.Add(""" & f.Name & """, " & f.Name & ", " & LATIRFramework.FieldTypesHelper.MapDBtype(LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True)) & ")"
                        End If
                    End If
                End If
        ...
        '  s = s & vbCrLf & "        Public Overrides Sub Unpack(ByVal dr As DataRow)"
        '  s = s & vbCrLf & "            on error resume next  "
        '  s = s & vbCrLf & "            If IsDBNull(dr(""SecurityStyleID"")) Then"
        '  s = s & vbCrLf & "                SecureStyleID = System.guid.Empty"
        '  s = s & vbCrLf & "            Else"
        '  s = s & vbCrLf & "                SecureStyleID = dr(""SecurityStyleID"")"
        '  s = s & vbCrLf & "            End If"
        '  s = s & vbCrLf & ""
        ...
        '        s = s & vbCrLf & "          if isdbnull(dr(""" & f.name & """)) then"
        '        s = s & vbCrLf & "            m_" & f.name & " = System.GUID.Empty"
        '        s = s & vbCrLf & "          else"
        '        s = s & vbCrLf & "            m_" & f.name & "=dr(""" & f.name & """)"
        '        s = s & vbCrLf & "          end if "
        '      Else
            ...
            If ft.TypeStyle <> MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Element_oformleniy Then
                ...
                Else
                    If UCase(LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True)) = "DATE" Then
                        ...
                        s = s & vbCrLf & "          end if "
                    ElseIf UCase(LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True)) = "STRING" Then
            ...
            If ft.TypeStyle <> MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Element_oformleniy Then
                ...
                If ft.TypeStyle = MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Ssilka Then
                    ...
                    Else
                        ...
                        If f.ReferenceType = MTZMetaModel.MTZMetaModel.enumReferenceType.ReferenceType_Na_stroku_razdela Then
                            ...
                            Else
                                ...
                                s = s & vbCrLf & "        Public Property " & f.Name & "() As LATIR.Document.docrow_base"
                                s = s & vbCrLf & "            Get"
                                ...
                                s = s & vbCrLf & "            End Get"
                                s = s & vbCrLf & "            Set(ByVal Value As LATIR.Document.docrow_base )"
                                s = s & vbCrLf & "                LoadFromDatabase()"
                    ...
                    If f.ReferenceType = MTZMetaModel.MTZMetaModel.enumReferenceType.ReferenceType_Na_ob_ekt_ Then
                        s = s & vbCrLf & MakeComment("Доступ к полю " & f.Caption)
                        s = s & vbCrLf & "        Public Property " & f.Name & "() As LATIR.Document.doc_base"
                        s = s & vbCrLf & "            Get"
                        s = s & vbCrLf & "                LoadFromDatabase()"
                        s = s & vbCrLf & "                " & f.Name & " = me.application.manager.GetInstanceObject(m_" & f.Name & ")"
                        s = s & vbCrLf & "                AccessTime = Now"
                        s = s & vbCrLf & "            End Get"
                        s = s & vbCrLf & "            Set(ByVal Value As LATIR.Document.doc_base )"
                        s = s & vbCrLf & "                LoadFromDatabase()"
                        s = s & vbCrLf & "                if not  Value is nothing then"
                        s = s & vbCrLf & "                  m_" & f.Name & " = Value.id"
                        s = s & vbCrLf & "                else"
                ...
                Else
                    If LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, f.FieldType.ID, tid, True) <> String.Empty Then
                        s = s & vbCrLf & MakeComment("Доступ к полю " & f.Caption)
                        s = s & vbCrLf & "        Public Property " & f.Name & "() As " & LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True)
                        s = s & vbCrLf & "            Get"
                        s = s & vbCrLf & "                LoadFromDatabase()"
                        s = s & vbCrLf & "                " & f.Name & " = m_" & f.Name & ""
                        s = s & vbCrLf & "                AccessTime = Now"
                        s = s & vbCrLf & "            End Get"
                        s = s & vbCrLf & "            Set(ByVal Value As " & LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True) & " )"
                        s = s & vbCrLf & "                LoadFromDatabase()"
        ...
        For i = 1 To p.FIELD.Count
            ...
            If ft.TypeStyle <> MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Element_oformleniy Then
                ...
                Else
                    ...
                    ElseIf UCase(LATIRFramework.FieldTypesHelper.MapFieldTypeToPhysicalType(m, ft.ID, tid, True)) = "GUID" Then
                        s = s & vbCrLf & "            m_" & f.Name & " =new system.guid(node.Attributes.GetNamedItem(""" & f.Name & """).Value)"
                    ElseIf UCase(ft.Name) = "IMAGE" Or _
        ...
        s = s & vbCrLf & "        Public Overrides Function GetDocCollection_Base(ByVal Index As Long) As LATIR.Document.DocCollection_Base"
        ...
        LATIRFramework.StringHelper.SetExt(o, p.Name, "vb")
        o.Block = "code"
        o.OutNL(s)
