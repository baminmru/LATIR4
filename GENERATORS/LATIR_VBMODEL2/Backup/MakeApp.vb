...
Module MakeApp
    ...
    Public Sub MakeApps(ByRef tid As String, ByRef m As MTZMetaModel.MTZMetaModel.Application, ByRef ot As MTZMetaModel.MTZMetaModel.OBJECTTYPE, ByRef o As LATIRGenerator.Response)
        ...
        s = s & vbCrLf & "Imports LATIR.Document"
        s = s & vbCrLf & ""
        ...
        s = s & vbCrLf & "        Inherits LATIR.Document.Doc_Base"
        s = s & vbCrLf & ""
        ...
        s = s & vbCrLf & "        Protected Overrides Function FindInCollections(ByVal Table As String, ByVal InstID As String) As LATIR.Document.DocRow_Base"
        s = s & vbCrLf & "        FindInCollections = Nothing"
        s = s & vbCrLf & "            dim mFindInCollections As LATIR.Document.DocRow_Base"
        For i = 1 To ot.PART.Count
            If ot.PART.Item(i).PartType = MTZMetaModel.MTZMetaModel.enumPartType.PartType_Derevo Or _
                           ot.PART.Item(i).PartType = MTZMetaModel.MTZMetaModel.enumPartType.PartType_Kollekciy Or _
                           ot.PART.Item(i).PartType = MTZMetaModel.MTZMetaModel.enumPartType.PartType_Rassirenie_s_dannimi Or _
        ...
        LATIRFramework.StringHelper.SetExt(o, "Application", "vb")
        ...
        For i = 1 To m.FIELDTYPE.Count
            With m.FIELDTYPE.Item(i)
                If .TypeStyle = MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Perecislenie Then
                    ...
                    s = s & vbCrLf & "public enum enum" & LATIRFramework.FieldTypesHelper.MakeValidName(.Name) & "'" & LATIRFramework.StringHelper.NoLF(.the_Comment)
                    For j = 1 To .ENUMITEM.Count
                        s = s & vbCrLf & "  " & LATIRFramework.FieldTypesHelper.MakeValidName(.Name) & "_" & LATIRFramework.FieldTypesHelper.MakeValidName(.ENUMITEM.Item(j).Name) & "=" & .ENUMITEM.Item(j).NameValue & "' " & .ENUMITEM.Item(j).Name
                    Next
                    s = s & vbCrLf & "end enum "
                End If
            End With
