...
bye:
    ...
    Private Sub KernelProcs()
        ...
        SQL.putBuf("     return ;")
        SQL.putBuf("   end if;")
        SQL.putBuf("   MTZ.Kernel.instance_ISLOCKED (acursession=>acursession,aROWID=>ainstanceid,aIsLocked=>aaccess  );")
        ...
        SQL.putBuf("        select 'begin MTZ.' || aobjtype || '.' || DeleteProc ||'(:1,:2); end;' into atmpstr from typelist where name = aobjtype;")
        SQL.putBuf("        If Not atmpstr Is Null Then")
        SQL.putBuf("            EXECUTE IMMEDIATE atmpstr USING acursession,ainstanceid;")
        SQL.putBuf("        end if;")
        ...
        SQL.putBuf("      select 'begin MTZ.' || aobjtype || '.' || HCLProc || '(:1,:2,:3); end;' into atmpstr from typelist where name = aobjtype;")
        SQL.putBuf("      If Not atmpstr Is Null Then")
        ...
        SQL.putBuf("    then")
        SQL.putBuf("      select  objtype into aobjtype from instance where instanceid=arowid ;")
        SQL.putBuf("      select  'begin MTZ.' || aobjtype || '.' || propagateProc || '(:1,:2); end;' into atmpstr from typelist where name = aobjtype;")
        SQL.putBuf("      If Not atmpstr Is Null Then")
        ...
        SQL.putBuf("End if;")
        SQL.putBuf(" select  usersid into auserID from  the_session where the_sessionid=acursession;")
        SQL.putBuf(" MTZ.Kernel.Instance_ISLOCKED (aCURSESSION,aROWID,aISLocked);")
        ...
        SQL.putBuf(" then")
        SQL.putBuf("  MTZ.Kernel.Instance_HCL( acursession,aRowID,aisLocked);")
        SQL.putBuf("  if aIsLocked >=3")
        SQL.putBuf("  then")
