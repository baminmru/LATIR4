...
bye:
    ...
    Private Function ReferenceCheck(ByVal os As PART, ByVal f As FIELD) As String
        ...
        If f.ReferenceType = enumReferenceType.ReferenceType_Na_stroku_razdela Then
            ...
            s.putBuf("  then")
            s.putBuf("    raise_application_error(-20000,'Отсутствует строка в таблице (" & VF(CType(f.RefToPart, PART).Name) & "), на которую установлена ссылка.  Раздел=" & VF(os.Name) & " поле=" & VF(f.Name) & "');")
            s.putBuf("    if aatrancount>0 rollback tran")
            s.putBuf("    return")
            s.putBuf("  end")
        End If
