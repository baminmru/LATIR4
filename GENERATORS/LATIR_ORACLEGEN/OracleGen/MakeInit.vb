...
Public Class MakeInit
    ...
    Public Sub Run()
        ...
        s.putBuf("BEGIN")
        s.putBuf("insert into instance(InstanceID, OBJTYPE, Name) values(ainstid, lower('MTZSYSTEM'),'Системная информация');")
        s.putBuf("Exception")
        s.putBuf("End;")

        s.putBuf("BEGIN")
        s.putBuf("insert into instance(InstanceID,OBJTYPE,Name) values(asecid, lower('mtzusers'), 'Пользователи и группы');")
        s.putBuf("Exception")
        s.putBuf("End;")

        s.putBuf("BEGIN")
        s.putBuf("delete from users where login='supervisor';")
        s.putBuf("Exception")
        s.putBuf("End;")
        ...
        For i = 1 To m.OBJECTTYPE.Count
            Dim tmps As String
            tmps = m.OBJECTTYPE.Item(i).Name.ToLower
            s.putBuf("insert into typelist( typelistid,name,DeleteProc, HCLProc, propagateProc) values(newid(),'" & tmps & "', '" & tmps & "_DELETE', '" & tmps & "_HCL', '" & tmps & "_propagate');")
        Next
        ...
        For i = 1 To m.OBJECTTYPE.Count
            ...
            If Not m.OBJECTTYPE.Item(i).ChooseView Is Nothing Then
                Dim tmps As String
                tmps = m.OBJECTTYPE.Item(i).Name.ToLower
                s.putBuf("" + SchemaName + ".Kernel.SysOptions_SAVE ( aSysOptionsid=>'" & GetMap2(m.OBJECTTYPE.Item(i).Name & "_TDEFVIEW") & "', aName=>'" & tmps & "', aValue=>'" & CType(m.OBJECTTYPE.Item(i).ChooseView, PARTVIEW).the_Alias.ToLower & "', aOptionType=>'TDEFVIEW');")
            End If
    ...
    Private Function MapPartView(ByVal os As PART) As String
        ...
        For i = 1 To os.PARTVIEW.Count
            If os.PARTVIEW.Item(i).ForChoose = enumBoolean.Boolean_Da Then
                s = s & vbCrLf & "" + SchemaName + ".Kernel.SysOptions_SAVE ( aSysOptionsid=>'" & GetMap2(os.Name & "_DEFVIEW") & "', aName=>'" & VF(os.Name) & "', aValue=>'" & os.PARTVIEW.Item(i).the_Alias.ToLower & "', aOptionType=>'DEFVIEW');"
                Exit For
            End If
        Next
    ...
    Private Function MapAndParent(ByVal os As PART) As String
        ...
        s = s & vbCrLf & "" + SchemaName + ".Kernel.SysOptions_SAVE  (aSysOptionsid=>'" & GetMap2(os.Name & "_structtype") & "', aName=>'" & VF(os.Name) & "', aValue=>'" & tn.ToLower & "', aOptionType=>'STRUCT_TYPE');"

        If TypeName(os.parent.parent) <> "OBJECTTYPE" Then
            s = s & vbCrLf & "" + SchemaName + ".Kernel.SysOptions_SAVE ( aSysOptionsid=>'" & GetMap2(os.Name & "_PARENT") & "', aName=>'" & VF(os.Name) & "', aValue=>'" & CType(os.Parent.Parent, PART).Name.ToLower() & "', aOptionType=>'PARENT');"
        End If
    ...
    Private Function MapViews(ByVal pv As PARTVIEW) As String
        Dim s As String
        s = "  " + SchemaName + ".Kernel.SysOptions_SAVE  (aSysOptionsid=>'" & GetMap2(pv.the_Alias & "_map") & "', aName=>'" & GetID(pv.ID.ToString) & "', aValue=.'v_" & pv.the_Alias.ToLower & "', aOptionType=.'MAP');"
        MapViews = s
    End Function
End Class