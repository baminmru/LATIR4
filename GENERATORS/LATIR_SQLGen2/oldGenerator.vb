...
bye:
    ...
    Private Sub MakeViews(ByRef pv As MTZMetaModel.MTZMetaModel.PARTVIEW, ByRef Lang As String)
        Debug.Print("SQLGEN.MakeViews:start " & pv.Name)
        Dim s As Writer
        ...
        o.OutNL("GO")
        s = Nothing
        Debug.Print("SQLGEN.MakeViews:done " & pv.Name)
        Exit Sub
bye:
        log = log & vbCrLf & "ERROR-->" & Err.Description & "<--ERROR"
        Debug.Print(Err.Description)
    ...
    'UPGRADE_NOTE: pv was upgraded to pv. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="A9E4979A-37FA-4718-9994-97DD76ED70A7"'
    Private Sub MakeLinkedView(ByRef pv As MTZMetaModel.MTZMetaModel.PARTVIEW, ByRef s As Writer, ByRef from As String, ByRef log As String, ByRef group As String, Optional ByRef Lang As String = "")
        Dim i As Integer
        ...
        For i = 1 To pv.PARTVIEW_LNK.Count
            PVD = pv.PARTVIEW_LNK.Item(i).TheView
            ...
            'MLF
            Dim part As MTZMetaModel.MTZMetaModel.PARTVIEW_LNK = pv.PARTVIEW_LNK.Item(i)
            ...
            If pv.PARTVIEW_LNK.Item(i).RefType = MTZMetaModel.MTZMetaModel.enumJournalLinkType.JournalLinkType_Ssilka_na_ob_ekt Then
                from = CStr((from & vbCrLf & " left join ") + PVD.Parent.Parent.Name + (" on ") + (root2.Name) + (".InstanceID=") + pv.Parent.Parent.Name + (".") + fld.Name + (" "))
            ElseIf pv.PARTVIEW_LNK.Item(i).RefType = MTZMetaModel.MTZMetaModel.enumJournalLinkType.JournalLinkType_Ssilka_na_stroku Then
                from = CStr((from & vbCrLf & " left join ") + PVD.Parent.Parent.Name + (" on ") + (root2.Name) + (".InstanceID=") + pv.Parent.Parent.Name + (".") + fld.Name + (" "))
            ElseIf pv.PARTVIEW_LNK.Item(i).RefType = MTZMetaModel.MTZMetaModel.enumJournalLinkType.JournalLinkType_Svyzka_InstanceID_OPNv_peredlah_ob_ektaCLS Then
                'UPGRADE_WARNING: Couldn't resolve default property of object PVD.Parent.Parent. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
                ...
                from = CStr((from & vbCrLf & " left join ") + PVD.Parent.Parent.Name + (" on ") + pv.Parent.Parent.Name + (".InstanceID=") + PVD.Parent.Parent.Name + (".") + ("InstanceID "))
            ElseIf pv.PARTVIEW_LNK.Item(i).RefType = MTZMetaModel.MTZMetaModel.enumJournalLinkType.JournalLinkType_Svyzka_ParentStructRowID__OPNv_peredlah_ob_ektaCLS Then
                'UPGRADE_WARNING: Couldn't resolve default property of object PVD.Parent.Parent. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
                'UPGRADE_WARNING: Couldn't resolve default property of object pv.Parent.Parent. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
                from = CStr((from & vbCrLf & " left join ") + PVD.Parent.Parent.Name + (" on ") + pv.Parent.Parent.Name + (".") + pv.Parent.Parent.Name + (".") + ("=") + PVD.Parent.Parent.Name + (".") + ("ParentStructRowID "))
            Else
                Exit For
            End If
