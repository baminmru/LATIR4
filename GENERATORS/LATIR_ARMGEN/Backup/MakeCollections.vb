...
Module MakeCollections
    ...
    Public Sub MakeFGView(ByRef tid As String, ByRef m As MTZMetaModel.MTZMetaModel.Application, ByRef ot As MTZFltr.MTZFltr.Application, ByRef p As MTZMetaModel.MTZMetaModel.PART, ByRef o As LATIRGenerator.Response, ByVal mode As String)
        Dim s As String = String.Empty
        Dim i As Integer
        Dim f As MTZMetaModel.MTZMetaModel.FIELD
        Dim ft As MTZMetaModel.MTZMetaModel.FIELDTYPE
        Dim n1 As String
        Dim ifs As LATIRFramework.ObjectHelper.InterfaceType
        ...
        ifs = LATIRFramework.ObjectHelper.AnalyzeInterfaceForGUI(p, mode)
        ...
        Select Case ifs
            ...
            Case LATIRFramework.ObjectHelper.InterfaceType.InterfaceTypeExtention
                ...
                If Not ei Is Nothing Then
                    ...
                    s = s & vbCrLf & "    Public Sub Attach(ByVal docItem As LATIR.Document.Doc_Base, ByVal gm As LATIRGuiManager.LATIRGuiManager,byval ReadOnlyView as boolean) Implements LATIRGUIControls.IViewPanel.Attach"
                    ...
                    prest = LATIRFramework.ObjectHelper.GetPartRestriction(p, mode)
            ...
            Case LATIRFramework.ObjectHelper.InterfaceType.InterfaceTypePanel ' "panel"
                ...
                '
            Case LATIRFramework.ObjectHelper.InterfaceType.InterfaceTypeGrid  ' "grid"
                ...
                s = s & vbCrLf & "    'It can be modified using the Windows Form Designer."
                s = s & vbCrLf & "    'Do not modify it using the code editor."
                s = s & vbCrLf & "    Friend WithEvents gv As LATIRGuiControls.GridView"
                s = s & vbCrLf & "    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()"
                s = s & vbCrLf & "        Me.gv = New LATIRGuiControls.GridView"
                s = s & vbCrLf & "        Me.SuspendLayout()"
                s = s & vbCrLf & "        '"
                s = s & vbCrLf & "        'gv"
                s = s & vbCrLf & "        '"
                ...
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Load table settings")
                ...
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Обслуживание редактирования Edit action")
                ...
                s = s & vbCrLf & "        If gui.ShowPartEditForm(""" & mode & """, CType(ed, Latir.Document.DocRow_Base), mReadOnly ) = True Then"
                s = s & vbCrLf & "            Dim dt As DataTable"
                s = s & vbCrLf & "            dt = item." & p.Name & ".GetDataTable()"
                s = s & vbCrLf & "            dt.TableName = """ & p.Name & """"
                s = s & vbCrLf & "            gv.SetData (dt)"
                ...
                s = s & vbCrLf & "    End Sub"
                s = s & vbCrLf & ""
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Обслуживание удаления Delete action")
                ...
                s = s & vbCrLf & "    End Sub"
                s = s & vbCrLf & ""
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Обслуживание создания записи Create action")
                ...
                s = s & vbCrLf & "        Dim dt As DataTable"
                s = s & vbCrLf & "        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)"
                s = s & vbCrLf & "        If gui.ShowPartEditForm(""" & mode & """, CType(ed,Latir.Document.DocRow_Base) ) = True Then"
                s = s & vbCrLf & "            dt = item." & p.Name & ".GetDataTable()"
                s = s & vbCrLf & "            dt.TableName = """ & p.Name & """"
                s = s & vbCrLf & "            gv.SetData (dt)"
                s = s & vbCrLf & "        Else"
                ...
                s = s & vbCrLf & "    End Sub"
                s = s & vbCrLf & ""
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Обслуживание кнопки <обновить> refresh action")
            ...
            Case LATIRFramework.ObjectHelper.InterfaceType.InterfaceTypeTree  ' "tree"
                s = s & vbCrLf & "Public Class view" & p.Name & mode
                s = s & vbCrLf & "    Inherits System.Windows.Forms.UserControl"
                s = s & vbCrLf & "    Implements LATIRGUIControls.IViewPanel"
                ...
                s = s & vbCrLf & "    'It can be modified using the Windows Form Designer."
                s = s & vbCrLf & "    'Do not modify it using the code editor."
                s = s & vbCrLf & "    Friend WithEvents TreeView1 As LATIRGuiControls.TreeView"
                s = s & vbCrLf & "    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()"
                s = s & vbCrLf & "        Me.TreeView1 = New LATIRGuiControls.TreeView"
                s = s & vbCrLf & "        Me.SuspendLayout()"
                s = s & vbCrLf & "        '"
                ...
                s = s & vbCrLf & "#End Region"
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Reference to the document, that serve control")
                's = s & vbCrLf & "    Public item As " & ot.Name & ".Application"
                s = s & vbCrLf & "    Public item As " & ot.Name & ".Application"
                s = s & vbCrLf & "    Private mReadOnly as boolean"
                s = s & vbCrLf & "    Public GuiManager As LATIRGuiManager.LATIRGuiManager"

                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Control initialization")
                s = s & vbCrLf & "    Public Sub Attach(ByVal docItem As LATIR.Document.Doc_Base, ByVal gm As LATIRGuiManager.LATIRGuiManager, byval ReadOnlyView as boolean )  Implements LATIRGUIControls.IViewPanel.Attach"
                s = s & vbCrLf & "        mReadOnly = ReadOnlyView"
                ...
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Load column settings")
                ...
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Load data to tree")
                ...
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Обслуживание добавления в корневой элемент")
                s = s & vbCrLf & "    Private Sub TreeView1_OnAddRoot(Byref OK As Boolean) Handles TreeView1.OnTreeAddRoot"
                s = s & vbCrLf & "      if not mReadOnly then"
                s = s & vbCrLf & "        Dim ed As " & ot.Name & "." & p.Name
                s = s & vbCrLf & "        ed = item." & p.Name & ".Add"
                s = s & vbCrLf & "        Dim gui As LATIRGuiManager.Doc_GUIBase"
                s = s & vbCrLf & "        Dim dt As DataTable"
                s = s & vbCrLf & "        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)"
                s = s & vbCrLf & "        If gui.ShowPartEditForm(""" & mode & """, CType(ed, Latir.Document.DocRow_Base )) = True Then"
                ...
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Обслуживание редактирования")
                ...
                s = s & vbCrLf & "        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)"
                s = s & vbCrLf & "        If gui.ShowPartEditForm(""" & mode & """, CType(ed, Latir.Document.DocRow_Base ), mReadOnly) = True Then"
                s = s & vbCrLf & "            if Typeof(ed.parent.parent) is " & ot.Name & "." & p.Name & " then"
                ...
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Обслуживание удаления")
                ...
                s = s & vbCrLf & LATIRFramework.StringHelper.MakeComment("Обслуживание добавления в уровень дерева")
            ...
            Case LATIRFramework.ObjectHelper.InterfaceType.InterfaceTypeTreeGrid ' "treegrid"
                child = Nothing

                p.PART.Sort = "sequence"
                For i = 1 To p.PART.Count
                    If LATIRFramework.ObjectHelper.IsPresent(p.PART.Item(i), mode) Then
                        child = p.PART.Item(i)
                        Exit For
                    End If
                Next
                ...
                s = s & vbCrLf & "    'It can be modified using the Windows Form Designer."
                s = s & vbCrLf & "    'Do not modify it using the code editor."
                s = s & vbCrLf & "    Friend WithEvents tgv As LATIRGuiControls.TreeGridView"
                s = s & vbCrLf & "    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()"
                s = s & vbCrLf & "        Me.tgv = New LATIRGuiControls.TreeGridView"
                s = s & vbCrLf & "        Me.SuspendLayout()"
                s = s & vbCrLf & "        '"
                s = s & vbCrLf & "        'tgv"
                s = s & vbCrLf & "        '"
            ...
            Case LATIRFramework.ObjectHelper.InterfaceType.InterfaceTypeGridGrid ' "gridgrid"
                ...
                s = s & vbCrLf & "    'It can be modified using the Windows Form Designer."
                s = s & vbCrLf & "    'Do not modify it using the code editor."
                s = s & vbCrLf & "    Friend WithEvents gv As LATIRGuiControls.GridGridView"
                s = s & vbCrLf & "    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()"
                s = s & vbCrLf & "        Me.gv = New LATIRGuiControls.GridGridView"
                s = s & vbCrLf & "        Me.SuspendLayout()"
                s = s & vbCrLf & "        '"
                s = s & vbCrLf & "        'gv"
                s = s & vbCrLf & "        '"
            ...
            Case LATIRFramework.ObjectHelper.InterfaceType.InterfaceTypeCommon ' "common"
                s = s & vbCrLf & "Public Class View" & p.Name & mode
                s = s & vbCrLf & "    Inherits System.Windows.Forms.UserControl"
                s = s & vbCrLf & "    Implements LATIRGUIControls.IViewPanel"
                ...
                s = s & vbCrLf & "    'It can be modified using the Windows Form Designer."
                s = s & vbCrLf & "    'Do not modify it using the code editor."
                s = s & vbCrLf & "    Friend WithEvents cv As LATIRGuiControls.ComplexTreeView"
                s = s & vbCrLf & "    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()"
                s = s & vbCrLf & "        Me.cv = New LATIRGuiControls.ComplexTreeView"
                s = s & vbCrLf & "        Me.SuspendLayout()"
                s = s & vbCrLf & "        '"
                s = s & vbCrLf & "        'cv"
                s = s & vbCrLf & "        '"
                ...
                s = s & vbCrLf & "      if not mReadOnly then"
                s = s & vbCrLf & "        Dim ed As LATIR.Document.DocRow_Base"
                s = s & vbCrLf & "        Dim edCol As LATIR.Document.DocCollection_Base"
                ...
                s = s & vbCrLf & "            If OK Then"
                s = s & vbCrLf & "              If TypeOf (pr) Is LATIR.Document.DocRow_Base Then"
                s = s & vbCrLf & "                 cv.TreeLoadData(pr.ID, PartName, ""Brief"", edcol.GetDataTable)"
                s = s & vbCrLf & "              End If"
                s = s & vbCrLf & "              If TypeOf (pr) Is LATIR.Document.Doc_Base Then"
                ...
                s = s & vbCrLf & "        Dim ed As LATIR.Document.DocRow_Base"
                s = s & vbCrLf & "        Dim pr As LATIR.Document.DocRow_Base"
                s = s & vbCrLf & "        ed = item.FindRowObject(PartName, ID)"
                s = s & vbCrLf & "        Dim gui As LATIRGuiManager.Doc_GUIBase"

                s = s & vbCrLf & "        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)"
                s = s & vbCrLf & "        If gui.ShowPartEditForm(""" & mode & """, CType(ed, Latir.Document.DocRow_Base ),mReadOnly ) = True Then"
                s = s & vbCrLf & "          If TypeOf (ed.Parent.Parent) Is LATIR.Document.DocRow_Base Then"
                s = s & vbCrLf & "            pr = ed.Parent.Parent"
                s = s & vbCrLf & "            cv.TreeLoadData(pr.ID, ed.PartName, ""Brief"", ed.Parent.GetDataTable)"
                s = s & vbCrLf & "          End If"
                s = s & vbCrLf & "          If TypeOf (ed.Parent.Parent) Is LATIR.Document.Doc_Base Then"
        ...
        LATIRFramework.StringHelper.SetExt(o, "View" & p.Name & mode, "vb")
        o.Block = "code"
        o.OutNL(s)
        Exit Sub
bye:
    ...
    Private Function MakeOnGridSetup(ByRef pc As MTZMetaModel.MTZMetaModel.PART_col, ByVal mode As String) As String
        ...
        For i = 1 To pc.Count
            ...
            For j = 1 To pc.Item(i).FIELD.Count
                s = s & vbCrLf & "            cs = New DataGridTextBoxColumn"
                s = s & vbCrLf & "            cs.ReadOnly = True"
                s = s & vbCrLf & "            cs.HeaderText = """ & LATIRFramework.StringHelper.NoLF(pc.Item(i).FIELD.Item(j).Caption) & """"
    ...
    Private Function MakeOnAdd(ByRef pc As MTZMetaModel.MTZMetaModel.PART_col, ByVal mode As String) As String
        ...
        For i = 1 To pc.Count
            ...
            If pc.Item(i).PartType = MTZMetaModel.MTZMetaModel.enumPartType.PartType_Derevo Then
                ...
                s = s & vbCrLf & "            gui = GuiManager.GetTypeGUI(ed.Application.TypeName)"
                s = s & vbCrLf & "            If gui.ShowPartEditForm("""", CType(ed, Latir.Document.DocRow_Base)) = True Then"
