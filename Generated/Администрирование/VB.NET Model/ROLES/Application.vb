...
''' <summary>
'''Реализация документа Каждый документ такого типа, описывает одну роль в рамках текущего бизнеса. Описание роли позволяет связать пользователя системы с теми рабочими местами, которые доступны для исполняющего роль сотрудника. Роль определяет  доступность пунктов меню, для каждого конкретного АРМ, а так же особенности работы документов.
''' </summary>
''' <remarks>
'''Роли
''' </remarks>
...
''' <summary>
''' Внутренняя переменная для раздела Доступные действия
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_ROLES_OPERATIONS As ROLES_OPERATIONS_col


''' <summary>
'''Свойство для доступа к строкам раздела Доступные действия
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property ROLES_OPERATIONS() As ROLES_OPERATIONS_col
            Get
                If m_ROLES_OPERATIONS Is Nothing Then
                    m_ROLES_OPERATIONS = New ROLES_OPERATIONS_col
                    m_ROLES_OPERATIONS.Application = Me
                    m_ROLES_OPERATIONS.Parent = Me
                    m_ROLES_OPERATIONS.Refresh()
                End If
                ROLES_OPERATIONS = m_ROLES_OPERATIONS
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Доступные приложения
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_ROLES_WP As ROLES_WP_col


''' <summary>
'''Свойство для доступа к строкам раздела Доступные приложения
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property ROLES_WP() As ROLES_WP_col
            Get
                If m_ROLES_WP Is Nothing Then
                    m_ROLES_WP = New ROLES_WP_col
                    m_ROLES_WP.Application = Me
                    m_ROLES_WP.Parent = Me
                    m_ROLES_WP.Refresh()
                End If
                ROLES_WP = m_ROLES_WP
            End Get
...
'''
''' </remarks>
  Private m_ROLES2_MODULE As ROLES2_MODULE_col
...
'''
''' </remarks>
        Public ReadOnly Property ROLES2_MODULE() As ROLES2_MODULE_col
            Get
                If m_ROLES2_MODULE Is Nothing Then
                    m_ROLES2_MODULE = New ROLES2_MODULE_col
                    m_ROLES2_MODULE.Application = Me
                    m_ROLES2_MODULE.Parent = Me
                    m_ROLES2_MODULE.Refresh()
                End If
                ROLES2_MODULE = m_ROLES2_MODULE
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Доступные документы
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_ROLES_DOC As ROLES_DOC_col


''' <summary>
'''Свойство для доступа к строкам раздела Доступные документы
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property ROLES_DOC() As ROLES_DOC_col
            Get
                If m_ROLES_DOC Is Nothing Then
                    m_ROLES_DOC = New ROLES_DOC_col
                    m_ROLES_DOC.Application = Me
                    m_ROLES_DOC.Parent = Me
                    m_ROLES_DOC.Refresh()
                End If
                ROLES_DOC = m_ROLES_DOC
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Отчёты
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_ROLES_REPORTS As ROLES_REPORTS_col


''' <summary>
'''Свойство для доступа к строкам раздела Отчёты
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property ROLES_REPORTS() As ROLES_REPORTS_col
            Get
                If m_ROLES_REPORTS Is Nothing Then
                    m_ROLES_REPORTS = New ROLES_REPORTS_col
                    m_ROLES_REPORTS.Application = Me
                    m_ROLES_REPORTS.Parent = Me
                    m_ROLES_REPORTS.Refresh()
                End If
                ROLES_REPORTS = m_ROLES_REPORTS
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Пользователи
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_ROLES_USER As ROLES_USER_col


''' <summary>
'''Свойство для доступа к строкам раздела Пользователи
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property ROLES_USER() As ROLES_USER_col
            Get
                If m_ROLES_USER Is Nothing Then
                    m_ROLES_USER = New ROLES_USER_col
                    m_ROLES_USER.Application = Me
                    m_ROLES_USER.Parent = Me
                    m_ROLES_USER.Refresh()
                End If
                ROLES_USER = m_ROLES_USER
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Отображение на группы
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_ROLES_MAP As ROLES_MAP_col


''' <summary>
'''Свойство для доступа к строкам раздела Отображение на группы
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property ROLES_MAP() As ROLES_MAP_col
            Get
                If m_ROLES_MAP Is Nothing Then
                    m_ROLES_MAP = New ROLES_MAP_col
                    m_ROLES_MAP.Application = Me
                    m_ROLES_MAP.Parent = Me
                    m_ROLES_MAP.Refresh()
                End If
                ROLES_MAP = m_ROLES_MAP
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Определение роли
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_ROLES_DEF As ROLES_DEF_col


''' <summary>
'''Свойство для доступа к строкам раздела Определение роли
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property ROLES_DEF() As ROLES_DEF_col
            Get
                If m_ROLES_DEF Is Nothing Then
                    m_ROLES_DEF = New ROLES_DEF_col
                    m_ROLES_DEF.Application = Me
                    m_ROLES_DEF.Parent = Me
                    m_ROLES_DEF.Refresh()
                End If
                ROLES_DEF = m_ROLES_DEF
            End Get
        End Property
...
''' </remarks>
        ...
        Public Overrides Sub Dispose()
            ...
            ROLES_USER.Dispose()
            ROLES_DOC.Dispose()
            ROLES_OPERATIONS.Dispose()
            ROLES_REPORTS.Dispose()
            ROLES2_MODULE.Dispose()
        End Sub
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = ROLES_DEF.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = ROLES_WP.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = ROLES_MAP.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = ROLES_USER.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = ROLES_DOC.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = ROLES_OPERATIONS.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = ROLES_REPORTS.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = ROLES2_MODULE.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("ROLES_DEF_COL")
            ROLES_DEF.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("ROLES_WP_COL")
            ROLES_WP.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("ROLES_MAP_COL")
            ROLES_MAP.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("ROLES_USER_COL")
            ROLES_USER.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("ROLES_DOC_COL")
            ROLES_DOC.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("ROLES_OPERATIONS_COL")
            ROLES_OPERATIONS.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("ROLES_REPORTS_COL")
            ROLES_REPORTS.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("ROLES2_MODULE_COL")
            ROLES2_MODULE.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            ROLES_DEF.XMLSave(node, Xdom)
            ROLES_WP.XMLSave(node, Xdom)
            ROLES_MAP.XMLSave(node, Xdom)
            ROLES_USER.XMLSave(node, Xdom)
            ROLES_DOC.XMLSave(node, Xdom)
            ROLES_OPERATIONS.XMLSave(node, Xdom)
            ROLES_REPORTS.XMLSave(node, Xdom)
            ROLES2_MODULE.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    ROLES_DEF.BatchUpdate
    ROLES_WP.BatchUpdate
    ROLES_MAP.BatchUpdate
    ROLES_USER.BatchUpdate
    ROLES_DOC.BatchUpdate
    ROLES_OPERATIONS.BatchUpdate
    ROLES_REPORTS.BatchUpdate
    ROLES2_MODULE.BatchUpdate
End Sub
    End Class
End Namespace
