...
Public Class viewROLES_USERmain
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.ROLES_USER.GetDataTable()
        dt.TableName = "ROLES_USER"

        ts = New DataGridTableStyle
        ts.MappingName = "ROLES_USER"
        ts.ReadOnly = True
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As ROLES.ROLES.ROLES_USER
        ed = item.FindRowObject("ROLES_USER", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.ROLES_USER.Add
          Else
              ed = Item.ROLES_USER.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
