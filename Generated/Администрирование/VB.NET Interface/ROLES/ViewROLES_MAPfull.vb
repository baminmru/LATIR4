...
Public Class viewROLES_MAPfull
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.ROLES_MAP.GetDataTable()
        dt.TableName = "ROLES_MAP"

        ts = New DataGridTableStyle
        ts.MappingName = "ROLES_MAP"
        ts.ReadOnly = True
        ...
        cs.ReadOnly = True
        cs.HeaderText = "Группа"
        cs.MappingName = "TheGroup"
        cs.NullText = ""
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As ROLES.ROLES.ROLES_MAP
        ed = item.FindRowObject("ROLES_MAP", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.ROLES_MAP.Add
          Else
              ed = Item.ROLES_MAP.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
