...
#End Region
    ...
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnMasterGridEdit
        Dim ed As MTZMetaModel.MTZMetaModel.MTZAPP
        ed = item.FindRowObject("MTZAPP", ID)
    ...
    Private Sub gv_OnChildRefresh() Handles gv.OnChildGridRefresh
        Dim ed As MTZMetaModel.MTZMetaModel.MTZAPP
        ed = item.FindRowObject("MTZAPP", gv.GetMasterID)
        Dim dt As DataTable
        If Not ed Is Nothing Then
            dt = ed.ParentPackage.GetDataTable
        Else
            dt = New DataTable
        End If
    ...
    Private Sub gv_OnChildDel(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnChildGridDel
      if not mReadOnly then
        Dim parent As MTZMetaModel.MTZMetaModel.MTZAPP
        parent = item.FindRowObject("MTZAPP", gv.GetMasterID)
        Dim ed As MTZMetaModel.MTZMetaModel.ParentPackage
        ed = item.FindRowObject("ParentPackage", ID)
        If MsgBox("Удалить <" & ed.Brief & "> ?", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "Удаление строки") = MsgBoxResult.Yes Then
            OK = ed.Parent.Delete(ed.ID.ToString)
            If OK Then
                Dim dt As DataTable
                'parent.MTZAPP.Refresh()
                dt = parent.ParentPackage.GetDataTable()
                dt.TableName = "ParentPackage"
                gv.SetDataChild(dt)
            End If
        End If
