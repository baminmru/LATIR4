...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.the_Session.GetDataTable()
        dt.TableName = "the_Session"

        ts = New DataGridTableStyle
        ts.MappingName = "the_Session"
        ts.ReadOnly = True
        ...
        cs.MappingName = "ApplicationID"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Текущая роль пользователя"
        cs.MappingName = "UserRole"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Момент закрытия"
        cs.MappingName = "ClosedAt"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Закрыта"
        cs.MappingName = "Closed"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
        ...
        cs.MappingName = "Usersid"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Последнее подтверждение"
        cs.MappingName = "LastAccess"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Момент открытия"
        cs.MappingName = "StartAt"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Локализация"
        cs.MappingName = "Lang"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As MTZSystem.MTZSystem.the_Session
        ed = item.FindRowObject("the_Session", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.the_Session.Add
          Else
              ed = Item.the_Session.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
