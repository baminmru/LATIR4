...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.SysRefCache.GetDataTable()
        dt.TableName = "SysRefCache"

        ts = New DataGridTableStyle
        ts.MappingName = "SysRefCache"
        ts.ReadOnly = True
        ...
        cs.ReadOnly = True
        cs.HeaderText = "Тип кеширования"
        cs.MappingName = "CacheType"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Идентификатор владельца"
        cs.MappingName = "ObjectOwnerID"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Сессия"
        cs.MappingName = "SessionID"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "модуль"
        cs.MappingName = "modulename"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As MTZSystem.MTZSystem.SysRefCache
        ed = item.FindRowObject("SysRefCache", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.SysRefCache.Add
          Else
              ed = Item.SysRefCache.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
