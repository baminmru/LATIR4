...
''' <summary>
'''Реализация документа Документ, который содержит исчерпывающую информацию о структурных особенностях и базовом поведении документов.
''' </summary>
''' <remarks>
'''Спец.: Метамодель системы
''' </remarks>
...
''' </remarks>
        Protected Overrides Function MyTypeName() As String
            MyTypeName = "MTZMetaModel"
        End Function



''' <summary>
''' Внутренняя переменная для раздела Пакет генерации
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_GENPACKAGE As GENPACKAGE_col


''' <summary>
'''Свойство для доступа к строкам раздела Пакет генерации
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property GENPACKAGE() As GENPACKAGE_col
            Get
                If m_GENPACKAGE Is Nothing Then
                    m_GENPACKAGE = New GENPACKAGE_col
                    m_GENPACKAGE.Application = Me
                    m_GENPACKAGE.Parent = Me
                    m_GENPACKAGE.Refresh()
                End If
                GENPACKAGE = m_GENPACKAGE
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Локализация
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_LocalizeInfo As LocalizeInfo_col


''' <summary>
'''Свойство для доступа к строкам раздела Локализация
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property LocalizeInfo() As LocalizeInfo_col
            Get
                If m_LocalizeInfo Is Nothing Then
                    m_LocalizeInfo = New LocalizeInfo_col
                    m_LocalizeInfo.Application = Me
                    m_LocalizeInfo.Parent = Me
                    m_LocalizeInfo.Refresh()
                End If
                LocalizeInfo = m_LocalizeInfo
            End Get
        End Property
...
'''
''' </remarks>
  Private m_FIELDTYPE As FIELDTYPE_col
...
'''
''' </remarks>
        Public ReadOnly Property FIELDTYPE() As FIELDTYPE_col
            Get
                If m_FIELDTYPE Is Nothing Then
                    m_FIELDTYPE = New FIELDTYPE_col
                    m_FIELDTYPE.Application = Me
                    m_FIELDTYPE.Parent = Me
                    m_FIELDTYPE.Refresh()
                End If
...
''' <summary>
''' Внутренняя переменная для раздела Методы и процедуры
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_SHAREDMETHOD As SHAREDMETHOD_col


''' <summary>
'''Свойство для доступа к строкам раздела Методы и процедуры
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property SHAREDMETHOD() As SHAREDMETHOD_col
            Get
                If m_SHAREDMETHOD Is Nothing Then
                    m_SHAREDMETHOD = New SHAREDMETHOD_col
                    m_SHAREDMETHOD.Application = Me
                    m_SHAREDMETHOD.Parent = Me
                    m_SHAREDMETHOD.Refresh()
                End If
                SHAREDMETHOD = m_SHAREDMETHOD
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Тип объекта
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_OBJECTTYPE As OBJECTTYPE_col


''' <summary>
'''Свойство для доступа к строкам раздела Тип объекта
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property OBJECTTYPE() As OBJECTTYPE_col
            Get
                If m_OBJECTTYPE Is Nothing Then
                    m_OBJECTTYPE = New OBJECTTYPE_col
                    m_OBJECTTYPE.Application = Me
                    m_OBJECTTYPE.Parent = Me
                    m_OBJECTTYPE.Refresh()
                End If
...
''' <summary>
''' Внутренняя переменная для раздела Приложение
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_MTZAPP As MTZAPP_col


''' <summary>
'''Свойство для доступа к строкам раздела Приложение
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property MTZAPP() As MTZAPP_col
            Get
                If m_MTZAPP Is Nothing Then
                    m_MTZAPP = New MTZAPP_col
                    m_MTZAPP.Application = Me
                    m_MTZAPP.Parent = Me
                    m_MTZAPP.Refresh()
                End If
                MTZAPP = m_MTZAPP
...
''' </remarks>
        ...
        Public Overrides Sub Dispose()
            ...
            OBJECTTYPE.Dispose()
            FIELDTYPE.Dispose()
            SHAREDMETHOD.Dispose()
            GENPACKAGE.Dispose()
            LocalizeInfo.Dispose()
        End Sub
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = MTZAPP.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = OBJECTTYPE.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = FIELDTYPE.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = SHAREDMETHOD.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = GENPACKAGE.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = LocalizeInfo.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("MTZAPP_COL")
            MTZAPP.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("OBJECTTYPE_COL")
            OBJECTTYPE.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("FIELDTYPE_COL")
            FIELDTYPE.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("SHAREDMETHOD_COL")
            SHAREDMETHOD.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("GENPACKAGE_COL")
            GENPACKAGE.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("LocalizeInfo_COL")
            LocalizeInfo.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            MTZAPP.XMLSave(node, Xdom)
            OBJECTTYPE.XMLSave(node, Xdom)
            FIELDTYPE.XMLSave(node, Xdom)
            SHAREDMETHOD.XMLSave(node, Xdom)
            GENPACKAGE.XMLSave(node, Xdom)
            LocalizeInfo.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    MTZAPP.BatchUpdate
    OBJECTTYPE.BatchUpdate
    FIELDTYPE.BatchUpdate
    SHAREDMETHOD.BatchUpdate
    GENPACKAGE.BatchUpdate
    LocalizeInfo.BatchUpdate
End Sub
    End Class
End Namespace
