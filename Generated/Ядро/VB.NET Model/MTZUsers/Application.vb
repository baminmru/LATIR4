...
''' <summary>
'''Реализация документа Документ содержит списки пользователей и состав групп пользователей системы.
''' </summary>
''' <remarks>
...
'''
''' </remarks>
        Public ReadOnly Property Users() As Users_col
            Get
                If m_Users Is Nothing Then
                    m_Users = New Users_col
                    m_Users.Application = Me
                    m_Users.Parent = Me
                    m_Users.Refresh()
                End If
                Users = m_Users
...
''' <summary>
''' Внутренняя переменная для раздела Группы
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_Groups As Groups_col


''' <summary>
'''Свойство для доступа к строкам раздела Группы
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property Groups() As Groups_col
            Get
                If m_Groups Is Nothing Then
                    m_Groups = New Groups_col
                    m_Groups.Application = Me
                    m_Groups.Parent = Me
                    m_Groups.Refresh()
                End If
                Groups = m_Groups
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = Users.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = Groups.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("Users_COL")
            Users.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("Groups_COL")
            Groups.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            Users.XMLSave(node, Xdom)
            Groups.XMLSave(node, Xdom)
        End Sub
