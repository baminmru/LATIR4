...
''' <summary>
'''Реализация документа Документ заполняется системой автоматически. Он содержит журнал активности пользователей системы, в который заносится вся информация о действиях пользователей.
''' </summary>
''' <remarks>
'''Спец.: Системные данные
''' </remarks>
...
''' </remarks>
        Protected Overrides Function MyTypeName() As String
            MyTypeName = "MTZSystem"
        End Function



''' <summary>
''' Внутренняя переменная для раздела Сессия пользователя
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_the_Session As the_Session_col


''' <summary>
'''Свойство для доступа к строкам раздела Сессия пользователя
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property the_Session() As the_Session_col
            Get
                If m_the_Session Is Nothing Then
                    m_the_Session = New the_Session_col
                    m_the_Session.Application = Me
                    m_the_Session.Parent = Me
                    m_the_Session.Refresh()
                End If
                the_Session = m_the_Session
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Разрешенные владельцы
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_SysRefCache As SysRefCache_col


''' <summary>
'''Свойство для доступа к строкам раздела Разрешенные владельцы
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property SysRefCache() As SysRefCache_col
            Get
                If m_SysRefCache Is Nothing Then
                    m_SysRefCache = New SysRefCache_col
                    m_SysRefCache.Application = Me
                    m_SysRefCache.Parent = Me
                    m_SysRefCache.Refresh()
                End If
                SysRefCache = m_SysRefCache
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Журнал событий
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_SysLog As SysLog_col


''' <summary>
'''Свойство для доступа к строкам раздела Журнал событий
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property SysLog() As SysLog_col
            Get
                If m_SysLog Is Nothing Then
                    m_SysLog = New SysLog_col
                    m_SysLog.Application = Me
                    m_SysLog.Parent = Me
                    m_SysLog.Refresh()
                End If
                SysLog = m_SysLog
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = the_Session.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = SysRefCache.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = SysLog.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("the_Session_COL")
            the_Session.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("SysRefCache_COL")
            SysRefCache.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("SysLog_COL")
            SysLog.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            the_Session.XMLSave(node, Xdom)
            SysRefCache.XMLSave(node, Xdom)
            SysLog.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    the_Session.BatchUpdate
    SysRefCache.BatchUpdate
    SysLog.BatchUpdate
End Sub
    End Class
End Namespace
