...
''' </remarks>
    Public Overrides Function ShowPartEditForm(ByVal Mode As String, ByRef RowItem As LATIR2.Document.DocRow_Base, optional byval FormReadOnly as boolean = false) As Boolean
        ...
        If Mode = "" Then
            ...
            If RowItem.PartName.ToUpper = "FILTERS" Then
                Dim f As frmFilters
                f = New frmFilters
            ...
            If RowItem.PartName.ToUpper = "FILTERFIELDGROUP" Then
                Dim f As frmFilterFieldGroup
                f = New frmFilterFieldGroup
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            End If

            If RowItem.PartName.ToUpper = "FILETERFIELD" Then
                Dim f As frmFileterField
                f = New frmFileterField
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            End If
