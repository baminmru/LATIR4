...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.MTZExtRel.GetDataTable()
        dt.TableName = "MTZExtRel"

        ts = New DataGridTableStyle
        ts.MappingName = "MTZExtRel"
        ts.ReadOnly = True
        ...
        cs.ReadOnly = True
        cs.HeaderText = "Реализация"
        cs.MappingName = "ThePlatform"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Название класса"
        cs.MappingName = "TheClassName"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Название библиотеки"
        cs.MappingName = "TheLibraryName"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As MTZExt.MTZExt.MTZExtRel
        ed = item.FindRowObject("MTZExtRel", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.MTZExtRel.Add
          Else
              ed = Item.MTZExtRel.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
