...
''' <summary>
'''Реализация документа Документ описывает структуру фильтра, который используется для ограничения множества информации, которая отображается в журнале , отчете и т.п.
''' </summary>
''' <remarks>
'''Описание фильтра
''' </remarks>
...
'''
''' </remarks>
  Private m_FilterFieldGroup As FilterFieldGroup_col
...
'''
''' </remarks>
        Public ReadOnly Property FilterFieldGroup() As FilterFieldGroup_col
            Get
                If m_FilterFieldGroup Is Nothing Then
                    m_FilterFieldGroup = New FilterFieldGroup_col
                    m_FilterFieldGroup.Application = Me
                    m_FilterFieldGroup.Parent = Me
                    m_FilterFieldGroup.Refresh()
                End If
                FilterFieldGroup = m_FilterFieldGroup
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Фильтр
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_Filters As Filters_col


''' <summary>
'''Свойство для доступа к строкам раздела Фильтр
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property Filters() As Filters_col
            Get
                If m_Filters Is Nothing Then
                    m_Filters = New Filters_col
                    m_Filters.Application = Me
                    m_Filters.Parent = Me
                    m_Filters.Refresh()
                End If
                Filters = m_Filters
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = Filters.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = FilterFieldGroup.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("Filters_COL")
            Filters.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("FilterFieldGroup_COL")
            FilterFieldGroup.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            Filters.XMLSave(node, Xdom)
            FilterFieldGroup.XMLSave(node, Xdom)
        End Sub
