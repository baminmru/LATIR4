...
''' <summary>
'''Реализация документа Документ описывает структуру автоматизированного рабочего места и особенности его функционирования.
''' </summary>
''' <remarks>
'''Описание АРМ
''' </remarks>
...
''' <summary>
''' Внутренняя переменная для раздела Поведение журналов
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_ARMJournal As ARMJournal_col


''' <summary>
'''Свойство для доступа к строкам раздела Поведение журналов
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property ARMJournal() As ARMJournal_col
            Get
                If m_ARMJournal Is Nothing Then
                    m_ARMJournal = New ARMJournal_col
                    m_ARMJournal.Application = Me
                    m_ARMJournal.Parent = Me
                    m_ARMJournal.Refresh()
                End If
                ARMJournal = m_ARMJournal
            End Get
        End Property
...
''' <summary>
''' Внутренняя переменная для раздела Арм
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_WorkPlace As WorkPlace_col


''' <summary>
'''Свойство для доступа к строкам раздела Арм
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property WorkPlace() As WorkPlace_col
            Get
                If m_WorkPlace Is Nothing Then
                    m_WorkPlace = New WorkPlace_col
                    m_WorkPlace.Application = Me
                    m_WorkPlace.Parent = Me
                    m_WorkPlace.Refresh()
                End If
...
''' <summary>
''' Внутренняя переменная для раздела Список типов
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_ARMTypes As ARMTypes_col


''' <summary>
'''Свойство для доступа к строкам раздела Список типов
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property ARMTypes() As ARMTypes_col
            Get
                If m_ARMTypes Is Nothing Then
                    m_ARMTypes = New ARMTypes_col
                    m_ARMTypes.Application = Me
                    m_ARMTypes.Parent = Me
                    m_ARMTypes.Refresh()
                End If
                ARMTypes = m_ARMTypes
            End Get
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = WorkPlace.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = EntryPoints.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = ARMTypes.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = ARMJournal.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("WorkPlace_COL")
            WorkPlace.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("EntryPoints_COL")
            EntryPoints.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("ARMTypes_COL")
            ARMTypes.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("ARMJournal_COL")
            ARMJournal.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            WorkPlace.XMLSave(node, Xdom)
            EntryPoints.XMLSave(node, Xdom)
            ARMTypes.XMLSave(node, Xdom)
            ARMJournal.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    WorkPlace.BatchUpdate
    EntryPoints.BatchUpdate
    ARMTypes.BatchUpdate
    ARMJournal.BatchUpdate
End Sub
    End Class
End Namespace
