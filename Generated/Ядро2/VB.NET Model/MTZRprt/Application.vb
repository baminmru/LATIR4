...
''' <summary>
'''Реализация документа Документ описывает отчетные формы, которые используются в АРМ.
''' </summary>
''' <remarks>
'''Описание отчета
''' </remarks>
...
''' <summary>
''' Внутренняя переменная для раздела Секции
''' </summary>
''' <remarks>
'''
''' </remarks>
...
''' <summary>
'''Свойство для доступа к строкам раздела Секции
''' </summary>
''' <remarks>
'''
''' </remarks>
...
''' <summary>
''' Внутренняя переменная для раздела Формулы
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_RPTFormula As RPTFormula_col


''' <summary>
'''Свойство для доступа к строкам раздела Формулы
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property RPTFormula() As RPTFormula_col
            Get
                If m_RPTFormula Is Nothing Then
                    m_RPTFormula = New RPTFormula_col
                    m_RPTFormula.Application = Me
                    m_RPTFormula.Parent = Me
                    m_RPTFormula.Refresh()
                End If
                RPTFormula = m_RPTFormula
            End Get
        End Property
...
'''
''' </remarks>
  Private m_Reports As Reports_col
...
'''
''' </remarks>
        Public ReadOnly Property Reports() As Reports_col
            Get
                If m_Reports Is Nothing Then
                    m_Reports = New Reports_col
                    m_Reports.Application = Me
                    m_Reports.Parent = Me
                    m_Reports.Refresh()
                End If
                Reports = m_Reports
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = Reports.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = RPTStruct.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = RPTFormula.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
            ...
            e_list = node.SelectNodes("Reports_COL")
            Reports.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("RPTStruct_COL")
            RPTStruct.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("RPTFormula_COL")
            RPTFormula.XMLLoad(e_list, LoadMode)
        End Sub
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            Reports.XMLSave(node, Xdom)
            RPTStruct.XMLSave(node, Xdom)
            RPTFormula.XMLSave(node, Xdom)
        End Sub
