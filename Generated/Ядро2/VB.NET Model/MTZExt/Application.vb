...
''' <summary>
'''Реализация документа Каталог функциональных расширений системы (ADDINS). 
''' </summary>
''' <remarks>
...
'''
''' </remarks>
  Private m_MTZExt_def As MTZExt_def_col
...
'''
''' </remarks>
        Public ReadOnly Property MTZExt_def() As MTZExt_def_col
            Get
                If m_MTZExt_def Is Nothing Then
                    m_MTZExt_def = New MTZExt_def_col
                    m_MTZExt_def.Application = Me
                    m_MTZExt_def.Parent = Me
                    m_MTZExt_def.Refresh()
                End If
                MTZExt_def = m_MTZExt_def
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Реализации расширения
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_MTZExtRel As MTZExtRel_col


''' <summary>
'''Свойство для доступа к строкам раздела Реализации расширения
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property MTZExtRel() As MTZExtRel_col
            Get
                If m_MTZExtRel Is Nothing Then
                    m_MTZExtRel = New MTZExtRel_col
                    m_MTZExtRel.Application = Me
                    m_MTZExtRel.Parent = Me
                    m_MTZExtRel.Refresh()
                End If
                MTZExtRel = m_MTZExtRel
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = MTZExt_def.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = MTZExtRel.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("MTZExt_def_COL")
            MTZExt_def.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("MTZExtRel_COL")
            MTZExtRel.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            MTZExt_def.XMLSave(node, Xdom)
            MTZExtRel.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    MTZExt_def.BatchUpdate
    MTZExtRel.BatchUpdate
End Sub
    End Class
End Namespace
