...
''' </remarks>
        Protected Overrides Function MyTypeName() As String
            MyTypeName = "STDInfoStore"
        End Function



''' <summary>
''' Внутренняя переменная для раздела Папка
''' </summary>
''' <remarks>
'''
''' </remarks>
...
''' <summary>
'''Свойство для доступа к строкам раздела Папка
''' </summary>
''' <remarks>
'''
''' </remarks>
...
'''
''' </remarks>
  Private m_InfoStoreDef As InfoStoreDef_col
...
'''
''' </remarks>
        Public ReadOnly Property InfoStoreDef() As InfoStoreDef_col
            Get
                If m_InfoStoreDef Is Nothing Then
                    m_InfoStoreDef = New InfoStoreDef_col
                    m_InfoStoreDef.Application = Me
                    m_InfoStoreDef.Parent = Me
                    m_InfoStoreDef.Refresh()
                End If
                InfoStoreDef = m_InfoStoreDef
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = Folder.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = InfoStoreDef.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
            ...
            e_list = node.SelectNodes("InfoStoreDef_COL")
            InfoStoreDef.XMLLoad(e_list, LoadMode)
        End Sub
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            ...
            InfoStoreDef.XMLSave(node, Xdom)
        End Sub
