...
''' </remarks>
        Protected Overrides Function MyTypeName() As String
            MyTypeName = "STDNumerator"
        End Function



''' <summary>
''' Внутренняя переменная для раздела Зоны
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_Num_zones As Num_zones_col


''' <summary>
'''Свойство для доступа к строкам раздела Зоны
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property Num_zones() As Num_zones_col
            Get
                If m_Num_zones Is Nothing Then
                    m_Num_zones = New Num_zones_col
                    m_Num_zones.Application = Me
                    m_Num_zones.Parent = Me
                    m_Num_zones.Refresh()
                End If
                Num_zones = m_Num_zones
            End Get
        End Property
...
'''
''' </remarks>
  Private m_Num_head As Num_head_col
...
'''
''' </remarks>
        Public ReadOnly Property Num_head() As Num_head_col
            Get
                If m_Num_head Is Nothing Then
                    m_Num_head = New Num_head_col
                    m_Num_head.Application = Me
                    m_Num_head.Parent = Me
                    m_Num_head.Refresh()
                End If
                Num_head = m_Num_head
            End Get
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = Num_head.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = Num_zones.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
            ...
            e_list = node.SelectNodes("Num_head_COL")
            Num_head.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("Num_zones_COL")
            Num_zones.XMLLoad(e_list, LoadMode)
        End Sub
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            Num_head.XMLSave(node, Xdom)
            Num_zones.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    Num_head.BatchUpdate
    Num_zones.BatchUpdate
End Sub
    End Class
End Namespace
